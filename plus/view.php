...
elseif($viewarcpwd>0)
{
	...
	if($viewarcpwd!=$postpwd)
	{
		$msgtitle = "您没有权限浏览文档：{$arctitle} ！<br />";
		$moremsg = "需要 <font color='red'>输入正确的密码才可以浏览</font> ！<br /><form name='form1' action='view.php' method='post'><input type='hidden' name='aid' value='".$arc->ArcID."' /><input type='text' name='postpwd' value='' /><input type='submit' value='提交密码' /></form>";
		include_once(DEDETEMPLATE.'/plus/view_msg.htm');
		exit();
	}
	else
	{
		if(($needRank>1 && $cfg_ml->M_Rank < $needRank && $arc->Fields['mid']!=$cfg_ml->M_ID))
		{
			...
			$memberTypes[0] = "游客或没权限会员";
			$msgtitle = "你没有权限浏览文档：{$arctitle} ！";
			$moremsg = "需要 <font color='red'>".$memberTypes[$needRank]."</font> 才能访问，你目前是：<font color='red'>".$memberTypes[$cfg_ml->M_Rank]."</font> ！";
			include_once(DEDETEMPLATE.'/plus/view_msg.htm');
			exit();
		}
	}
